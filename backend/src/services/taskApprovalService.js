  ...
  const [updatedSub] = await Promise.all([
    pool.query(
      `UPDATE "TaskSubmission" SET status = 'APPROVED', "reviewedById" = $1, "reviewNote" = $2,
       "rewardPaid" = $3, "reviewedAt" = now(), "autoApproved" = $4 WHERE id = $5 RETURNING *`,
      [reviewedById, note, submission.rewardAmount, !reviewedById, submission.id]
    ),
    pool.query('UPDATE "Task" SET "completedCount" = "completedCount" + 1 WHERE id = $1', [submission.taskId]),
  ]);
