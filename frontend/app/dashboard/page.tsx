    ...
    <div style={{ height: "100dvh", display: "flex", flexDirection: "column", background: "#0A1A12", overflow: "hidden" }}>
      ...
      {tab === "main" && (
        <div className="px-4 pb-6 w-full max-w-2xl mx-auto">
          ...
          {/* WhatsApp card */}
          {(siteSettings.whatsapp_number || siteSettings.whatsapp_group_link) && (
            <div className="mt-4 rounded-3xl p-5" style={{ background: "linear-gradient(135deg, #0a2318 0%, #071a10 100%)", border: "1.5px solid rgba(37,211,102,0.25)" }}>
              <div className="flex items-center gap-4 mb-1">
                <div style={{ width: 52, height: 52, borderRadius: 16, background: "rgba(37,211,102,0.12)", border: "1.5px solid rgba(37,211,102,0.3)", display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0 }}>
                ...
                <div style={{ flex: 1, minWidth: 0 }}>
                  <div style={{ fontSize: 15, fontWeight: 700, color: "#F5F2EA", marginBottom: 2 }}>WhatsApp</div>
                  <div style={{ fontSize: 12, color: "rgba(245,242,234,0.5)" }}>Get help or join our community</div>
                </div>
              </div>
              <div className="flex gap-2 mt-3">
                {siteSettings.whatsapp_number && (
                  <a href={`https://wa.me/${siteSettings.whatsapp_number.replace(/\D/g, "")}`} target="_blank" rel="noopener noreferrer"
                    className="flex-1 text-center" style={{ padding: "10px 0", borderRadius: 12, background: "#25D366", color: "#000", fontSize: 12, fontWeight: 800 }}>
                    Chat with us
                  </a>
                )}
                {siteSettings.whatsapp_group_link && (
                  <a href={siteSettings.whatsapp_group_link} target="_blank" rel="noopener noreferrer"
                    className="flex-1 text-center" style={{ padding: "10px 0", borderRadius: 12, background: "rgba(37,211,102,0.12)", border: "1px solid rgba(37,211,102,0.3)", color: "#25D366", fontSize: 12, fontWeight: 800 }}>
                    Join our Group
                  </a>
                )}
              </div>
